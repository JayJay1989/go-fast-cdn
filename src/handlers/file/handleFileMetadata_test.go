...
func TestHandleFileMetadata_NoError(t *testing.T) {
	// Arrange
	testFileName := uuid.NewString()
	testFileDir := filepath.Join(util.ExPath, "uploads", "files")
...
func TestHandleDocMetadata_NotFound(t *testing.T) {
	...
	require.Equal(t, result["error"], "File does not exist")
}
...
func TestHandleDocMetadata_NameNotProvided(t *testing.T) {
	...
	require.NoError(t, err)
	require.Contains(t, result, "error")
	require.Equal(t, result["error"], "File name is required")
}
